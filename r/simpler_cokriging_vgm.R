...
# Local Regression
local.srtm <- loess(log(c_elev) ~ s_dem, data = yel)
...
dem.ols.vgm <- vgm(model = "Nug", nugget = 170, range = 0, psill = 0)
dem.ols.fit <- fit.variogram(dem.ols.evar, dem.ols.vgm)

# Plot it
plot(dem.ols.evar, model = dem.ols.fit, main = "Variogram Model of OLS Residuals")
